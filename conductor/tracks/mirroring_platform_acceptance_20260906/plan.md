...
- [ ] T1 Freeze acceptance fixtures and failing regression tests. (AC1–AC3)
  - **Files**: `scripts/mirroring_acceptance.py`, `tests/browser/mirroring_acceptance.spec.js`, `tests/browser/playwright.config.js`, `package.json`, `package-lock.json`, `.github/workflows/mirroring-acceptance.yml`, `docs/MIRRORING_COMPATIBILITY.md`, `README.md`; this track’s plan, metadata and evidence.
  ...
  - **Verify**: `python3 scripts/mirroring_acceptance.py --help && npx --no-install playwright test --config tests/browser/playwright.config.js`; `python3 scripts/gate.py test`; full Conductor validation. For T1, record expected failing new assertions and require existing baseline tests to pass.
