...
def ssl_context(config, now=None):
    config = config or {}
    if config.get('mode') != 'client_certificate': return None
    if config.get('expires_at') is not None and (time.time() if now is None else now) >= float(config['expires_at']): return None
    certfile, keyfile = config.get('certificate_file'), config.get('key_file')
    if not certfile or not keyfile: raise ValueError('client_certificate requires certificate_file and key_file')
