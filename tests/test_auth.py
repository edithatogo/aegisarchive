...
class AuthBoundaries(unittest.TestCase):
    def test_missing_scope_does_not_send_credentials(self):
        self.assertEqual(request_headers({'mode':'cookies_env','source':'secret'}, 'https://example.test/'), {})

    def test_cookie_host_path_and_secure_boundaries(self):
        state = {'cookies': [{'name':'sid','value':'secret','domain':'example.test','path':'/app','secure':True}]}
        config = {'mode':'browser_session','source':json.dumps(state),'allowed_domains':['example.test']}
        self.assertEqual(request_headers(config,'https://example.test/app/a'), {'Cookie':'sid=secret'})
        for url in ('http://example.test/app','https://sub.example.test/app','https://example.test/apple'):
            self.assertEqual(request_headers(config,url), {})

    def test_certificate_does_not_load_outside_scope(self):
        config = {'mode':'client_certificate','allowed_domains':['example.test'],'certificate_file':'cert','key_file':'key'}
        with patch('cli.auth.ssl.create_default_context') as create:
            self.assertIsNone(ssl_context(config,url='https://other.test/'))
